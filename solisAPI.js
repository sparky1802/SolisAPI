...
//CREATE THE API REQUEST TO POST AND FETCH THE DATA
async function getDetails(canonicalizedResources, content, keyID, keySecret) {
    ...
    const message = await messageToSign(httpVerb, contentMD5, contentType, gmtDate, canonicalizedResources);
    ...
    //Fetch the data   
    const resultResponse = await fetch(hostID + canonicalizedResources + "?" + authorization, requestOptions)
        .then(response => {
